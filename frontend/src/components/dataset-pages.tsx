...
export function CleanContent({ dataset }: { dataset: string }) {
  ...
  return <>
    <h1>Dữ liệu sạch</h1>
    <p className="status-line">{data.state.label}</p>
    <div className="card"><TableSummary table={data.table} />
...
export function RoundContent({ dataset, round }: { dataset: string; round: string }) {
  ...
  return <>
    ...
    {data.answer ? <pre className="card">{JSON.stringify(data.answer, null, 2)}</pre> : <div className="card">Chưa có câu trả lời.</div>}
