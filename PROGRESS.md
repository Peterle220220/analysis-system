...
| 🟡 | **Việc 2 — mỗi skill một model** | `OpenRouterProvider` + `LlmPolicy.model` trong manifest. Một cổng tới ~424 model; đổi model là sửa **một dòng config**, không đụng code. Ngân sách **dùng chung** qua `for_model()` — chín agent vẫn một trần. `pricing.yaml` chặn model chưa khai giá. Đã đo 3 model free ×5 lần: chỉ `dots-3-note` đạt 5/5 (L69). **Còn lại: gán model cho từng manifest** |
